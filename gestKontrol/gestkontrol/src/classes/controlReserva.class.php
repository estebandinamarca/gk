...
class controlReserva
{
	...
	public function getReservasforDate($estadoValidacion=null,$fechaEntradaI,$fechaTopeEntrada,$idc=null)
	{
		$sql=conexionMySQLi::getInstance();
		$reserva = null;
		$estadoV = "Reservada";
		$filtro = " and idCliente = ?";
		$sentencia= "SELECT idReserva,idCliente,idOperador,idVisita,fechaEntrada,fechaSalida,tipoReserva,tipoFrecuencia,piso,oficina,estacionamientoAsignado,estadoValidacion,momentoValidacion,codigoReserva,patenteVehiculo FROM reserva WHERE estadoValidacion = ? AND fechaEntrada>= ? AND fechaEntrada<= ?";
		$horaInicio = " 00:00:00";
		...
		{
			$resulset = $sql->devDatos($sentencia,array($estadoV,$fechaEntradaI.$horaInicio,$fechaTopeEntrada.$horaTermino));
		}
