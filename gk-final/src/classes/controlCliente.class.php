...
class controlCliente
{
	public function getCliente($idCliente=null)
	{
		...
		$filtro= " ORDER BY nombreEmpresa ";
		if($idCliente!=null)
		{
			$busqueda = "WHERE idCliente = ?";
			$sentencia = $query.$busqueda.$filtro;
			$cliente = null;
			//die($sentencia);
	...
	public function getDetalleCliente($idClientes=null,$soloPisos=null,$estadoBusqueda=null) //si es nulo se asume que es solo piso, si no retorna los pisos asociados a sus respectivas oficinas 
	{
		...
		else if($idClientes!=null && $soloPisos==null && $estadoBusqueda==null) // Muestra todo los pisos y oficinas asociados a un cliente en particular
		{
		...
		else if($idClientes==null && $soloPisos!=null && $soloPisos>=-2) // esta funcion se realizara con el fin de buscar por los pisos las oficinas relacionadas a el
		{
			//print_r($soloPisos);
			$filtro="FROM detalleCliente WHERE piso = ? ORDER BY oficina ASC ";
			$sentencia = $sentencia.$filtro;
		...
		else if($idClientes == null && $soloPisos!=null && $soloPisos<-2) // muestra solo los pisos asociados a la tabla detalleCliente( solo 1 ves por piso).
		{
			//print_r($soloPisos);
		...
		else if($idClientes!=null && $soloPisos==null && $estadoBusqueda!=null)
		{
			
			$filtro="FROM detalleCliente WHERE idDetalleCliente = ? AND idCliente = ? ";
			$sentencia = $sentencia.$filtro;
			$resulset = $sql->devDatos($sentencia,array($estadoBusqueda,$idClientes));
			$resulset->bind_result($idDetalleCliente,$idCliente,$oficina,$piso,$orientacion,$telefono,$contacto,$email);
	...
	public function getestacionamieto($idCliente = null,$subterraneos = null,$idEsta = null,$soloAhora=null)
	{
		$sql = conexionMySQLi::getInstance();
		...
		if($idCliente!=null && $subterraneos==null && $idEsta==null && $soloAhora==null) // retorna todo los estacionamientos asociados a un cliente
		{
			...
			$resulset = $sql->devDatos($sentencia,array($idCliente));
			$resulset->bind_result($idEstacionamiento,$idCliente,$subterraneo,$numero,$estado,$prov,$visita);
			
			while($resulset->fetch())
			{
				$estacionamientos[]= new estacionamiento($idEstacionamiento,$idCliente,$subterraneo,$numero,$estado,$prov,$visita);
			}
		...
		if($idCliente==null && $subterraneos!=null && $subterraneos>2 && $idEsta==null)//retorna solos subterraneos .
		{
			...
			while($value = $resulset->fetch_assoc())
			{
				$estacionamientos[] = new estacionamiento($value["idEstacionamiento"],$value["idCliente"],$value["subterraneo"],$value["numero"],$value["estado"],$value["proveedor"],$value["visita"]);
			}
		}
		if($idCliente!=null && $soloAhora!=null)//retorna solos subterraneos de un cliente en particular .
		{
			//echo " solo";
			$filtro = "FROM estacionamiento Where idCliente = ? GROUP BY subterraneo desc";
			$sentencia = $sentencia.$filtro;
			//echo "aqui Antes";die($sentencia);
			$resulset = $sql->devDatos($sentencia,array($idCliente));
			$resulset->bind_result($idEstacionamiento,$idCliente,$subterraneo,$numero,$estado,$proveedor,$visita);
				
			while($resulset->fetch())
			{
				$estacionamientos[]= new estacionamiento($idEstacionamiento,$idCliente,$subterraneo,$numero,$estado,$proveedor,$visita);
			}
		}
	...
	public function actualizaEstaVisitaDeCliente($data)
	{
		$sql = conexionMySQLi::getInstance();
		$respuesta = -2;
		$sentencia = "UPDATE estacionamiento SET visita = ? Where idEstacionamiento = ?";
		$sql->ejecutarSentencia($sentencia,$data);
		$respuesta = 1;
		return $respuesta;
...
public function insertCliente($data)
	...
	//##############MICHAEL###########################
	public function listClienteHistory($idCliente=null)
	{
		$sql = conexionMySQLi::getInstance();
		//die($idCliente);
		$query = "SELECT idClienteHistory,rutp,dvp,nombre,direccion From clienteHistory GROUP BY nombre ";
		...
		else
		{
			...
			while($value = $resulset->fetch_assoc())
			{
				$cliente[] = new cliente($value["idClienteHistory"],$value["rutp"],$value["dvp"],$value["nombre"],$value["direccion"]);
			}
