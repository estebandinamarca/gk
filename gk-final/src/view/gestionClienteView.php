...
$(document).ready(function(){
	...
	public function getEditarPisoCliente($idCliente)
	{
		...
		$pisoall = controlCliente::getDetalleCliente(null,-3);
		$totalTabla = count(controlCliente::getDetalleCliente());
...
$(document).ready(function(){
	...
	public function geteditarEstacionamientosDivConfig($idCliente=null,$usuario=null)
	{
		?>
				
				<div data-role="page" id="editar-cliente-estacionamientos"> <!-- data-role="page" id="popup-editar-cliente-piso" -->
					<div data-role="content" id="content-editar-cliente-estacionamientos"></div>
				</div><!-- /page -->	
	...
	public function getEditEstacionamientoClientes($idCliente,$AloneCliente=null)
	{
		$pisoall = null;
		$totalTabla = null;
		//$pisosDelCliente = null;
		...
		$bandera = $idCliente;
		$pisoall = controlCliente::getestacionamieto($bandera,null,null,1);
		$fin = count($pisoall);
		//print_r($pisoall);
		//die();
		$itera = 0;
		$dataTotalEstaDeCliente = controlCliente::getestacionamieto($bandera);
		$totalTabla = count($dataTotalEstaDeCliente);
		//$dataArrStrig = $dataTotalEstaDeCliente;
		//
		$tmp = serialize($dataTotalEstaDeCliente);
		$tmp = urlencode($tmp);
		//
		$cliente = controlCliente::getClienteId($idCliente);
		$EstaxSub = null;
		$totalEstaxSub = 0;
		?>
			<h3>Inicio >> Editar Estacionamientos </h3>
			<hr>
			<form action="post" id="editar-cliente-su-estacionamiento">
			<h3 style="text-align:left;"  class="textoPisosClientes"> <?php //echo $cliente->getnombreEmpresa();?></h3>
			
			<div data-role="collapsible-set" data-content-theme="e">
				<?php 
				while($inicio<$fin)
				{
					$EstaxSub = controlCliente::getestacionamieto($bandera, $pisoall[$inicio]->getsubterraneo());
					//print_r($EstaxSub);
					//die();
				...
				?>
				<div data-role="collapsible" >
					<h3><?php echo "Subterraneo ".$pisoall[$inicio]->getsubterraneo();?></h3>
					  <div data-role="collapsible-set" data-mini="true" data-content-theme="c">
					  	...
					  	<h3><?php echo " ".$EstaxSub[0]->getnumero()."-".($EstaxSub[$totalEstaxSub-1]->getnumero());?></h3>
						  	...
						  	<?php 
						  		...
						  		{
						  			$name = "checkbox___".$estac->getidEstacionamiento();
						  			
						  	 ?>
							  	           	<input type="checkbox" name="<?php echo $name;?>" id="<?php echo $name;?>" class="custom" <?php echo $estac->getvisita()==1?"checked=checked": " ";?> />
			...
			<hr>
			<div class="">
			<input type="text" name="pisoEditEstacVisita" id="pisoEditEstacVisita" value="<?php echo $idCliente?>" style="display: none;"/>
			<input type="text" name="totaldetEstacVisita" id="totaldetEstacVisita" value="<?php echo $tmp;?>" style="display: none;"/>
			<fieldset class="ui-grid-a">
			<a href="#" data-role="button" data-theme="b" data-inline="true" id="EditarEstacSuCliente">Guardar</a>
			<a href="#" data-role="button" data-theme="d" onClick="location.replace('index.php');" data-icon="back" data-inline="true" data-transition='pop' rel='close'>Volver</a>
			</fieldset>
			</div>
			</form>
			...
			$(document).ready(function(){
				...
				$("#EditarEstacSuCliente").click(function(){
			
					var datos = $("#editar-cliente-su-estacionamiento").serialize();
					//alert("hola");
					$.ajax({
				  		   type: "POST", //Establecemos como se van a enviar los datos puede POST o GET
				  		   url: "validaIngresoCliente.php", //SCRIPT que procesara los datos, establecer ruta relativa o absoluta
